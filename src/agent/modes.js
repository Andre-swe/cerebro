...
async function execute(mode, agent, func, timeout=-1) {
    // Workers don't have a self_prompter
    ...
    let should_reprompt =
        ...
        (!agent.self_prompter || !agent.self_prompter.isActive()) && // self prompting is not on (workers don't have self_prompter)
...
export function initModes(agent) {
    ...
    // Load modes from prompter (for regular agents) or from settings (for workers)
    if (agent.prompter) {
        ...
        // Workers don't have a prompter - load modes from settings.profile directly
        if (settings.profile?.modes) {
            console.log(`[Modes] Loading worker modes from profile:`, settings.profile.modes);
            agent.bot.modes.loadJson(settings.profile.modes);
        } else {
            // Fallback: Enable essential worker modes by default
            console.log(`[Modes] No profile modes found, using defaults for worker`);
            agent.bot.modes.loadJson({
